...
contract("TestProductOrderBook", function() {
	...
	function order(market, amount, sender, expectedStake, expectedFee) {
		...
		function worker() {
			return dapp.order(market, amount, expectedStake, {from: sender}).then(function(result) {
				...
				assert.equal(a, amount, "Count should match");
			});
		}
	...
	it("Should be able to make an order", function() {
		var id;
		var amount = 1;
		var expectedStake = amount*price*stakeRate;
	...
	it("Should be able to make an order for more than one non-metered goods", function() {
		...
		return order(market_id, amount, client, expectedStake, expectedFee).then(function(_id) {
			id = _id;
			//console.log(id);
			return order(market_id, amount, provider, expectedStake, expectedFee);
		}).then(function(_id) {
			id = _id;
			//console.log(id);
		});
	...
	it("Should be able to make a stake offer larger than the minimum", function() {
		var id;
		var amount = 1;
		var expectedStake = price*amount*stakeRate + 1;
	...
	it("Given funds have been deposited, client and provider should be able to confirm order", function() {
		var id;
		var amount = 1;
		var stake = amount*price*stakeRate;
